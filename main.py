...
def shop():
    ecommerce_posts = db.session.execute(db.select(ECommercePost)).scalars().all()
    return render_template("shop.html", ecommerce_posts=ecommerce_posts, current_user=current_user)
